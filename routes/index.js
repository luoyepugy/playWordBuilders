...
module.exports=function(router,app) {
  ...
  router.get('/', function (req, res, next) {
    ...
    async.parallel({
      ...
      //   // res.render('index/index', {data: data.data});
      //  });
      // },
      funny:function(callback){
        db.collection('funny').find({},{limit:10}).sort({time:-1}).toArray(function(err,ret){
          callback(err,ret);
        });
      },
      daily: function(callback) {
        db.collection('daily').find({},{limit:10}).sort({time:-1}).toArray(function(err,ret){
          callback(err,ret);
        });
      },
      world:function(callback){
        db.collection('world').find({},{limit:10}).sort({time:-1}).toArray(function(err,ret){
          callback(err,ret);
        });
      },
      entertainment: function(callback) {
        db.collection('entertainment').find({},{limit:10}).sort({time:-1}).toArray(function(err,ret){
          callback(err,ret);
        });
      },
  ...
  router.get('/news', function (req, res, next) {
    db.collection('page_data').findOne({page: 'index'}, function (err, data) {
      res.render('index/news', {data: data.data});
    });
  });*/
