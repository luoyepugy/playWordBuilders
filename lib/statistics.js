...
module.exports = function(app){
    ...
    var getViewByHour = function(time,callback){
        collection.find({day:time[0],hour:time[1]}).toArray(function(err,ret){
        ...
        if(type=='hour'){
            ...
            async.whilst(function() {
                ...
                function(cb){
                           ...
                           collection.findOne({day:timeArr[count][0],hour:timeArr[count][1]},function(err,ret){
            ...
            async.whilst(function() {
                ...
                function(cb){
                    ...
                    collection.find({day:timeArr[count][0]}).toArray(function(err,ret){
    ...
    var getNewVisitors = function(timeArr,type,callback){
        if(type=='hour'){
            ...
            async.whilst(function() {
                ...
                function(cb){
                    collection.find({day:timeArr[count][0],newVisitor:true}).toArray(function(err,ret){
        ...
        async.whilst(function() {
            ...
            function(cb){
                if(count<timeArr.length){
                    collection.find({timestamp:{$gt:timeArr[count]*1,$lt:timeArr[0]*1},newVisitor:true}).toArray(function(err,ret){
                    ...
                    collection.find({timestamp:{$gt:timeArr[count-3]*1,$lt:timeArr[0]*1}}).toArray(function(err,ret){
    ...
    return {
        settings:function(newVisitor,uid,callback){
                    ...
                    collection.findOne({day:nowTime[0],hour:nowTime[1]},function(err,ret) {
                        //  console.log(ret);
                        if (err) {
                            console.log(err);
                        } else{
                            ...
                            if(newVisitor){var newNum=ret.newVisitor+1;}else{var newNum=ret.newVisitor}
                                ...
                                Ips.push(uid);
                            collection.update({day:nowTime[0],hour:nowTime[1]},{$set:{newVisitor:newNum,views:ret.views+1,Ips:Ips}},function(err,ret){
                                if(err)console.log(err);
                                console.log(ret);
                            });
                        }
